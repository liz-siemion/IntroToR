...
What about if we want to manually set the working directory directory or tell R to load a file from a specified location? How would we do that? We need to first know the file path...

## What is a file path?

File paths are addresses to different locations or files on a computer. Your computer uses a system of nested folders.

![Nested file system on your computer -- image from R for epidemiology (https://www.r4epi.com)](./docs/files/file_path_hierarchy.png){width=40%}

<br>

File paths are addresses to different locations or files within this nested framework. They represents the order of nested folders that the computer must go through to find that particular item. Each folder is separated by a slash. We can use `Absolute` or `Relative` file paths in R to locate files. Knowing the file path is important when you need to set your working directory. See [this website](https://www.r4epi.com/file-paths) for a detailed explanation.  
